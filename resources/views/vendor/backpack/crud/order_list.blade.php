  ...
  <div class="row">
    ...
    <div class="{{ $crud->getListContentClass() }}">
      <div class="">
        ...
        @endif
          <div class="box box-orderchina clearfix">
          <div class="list-order clearfix">
              <ul class="ls-tags list-inline hidden-xs">

                  @foreach(\App\Helpers::getOrderStatuses() as $key => $item)

                      <li>
                          <a href="{{ backpack_url('order?type='.$key) }}"
                             class="{{request()->input('type') == $key ? 'active' : ''}}">{{$item}}
                              @if(\App\Models\Order::where('order_status', $key)->count() > 0)
                                  <span class="badge ng-binding">
                        {{\App\Models\Order::where('order_status', $key)->count()}}
                      </span>
                              @endif
                          </a>
                      </li>
